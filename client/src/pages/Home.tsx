...
const packages = [
  { name: "Paket Syukur", price: "Mulai 1,9 jt", detail: "Untuk 1 ekor kambing", accent: "olive", items: ["Kambing sehat sesuai syariat", "Masakan olahan kambing", "Nasi box siap dibagikan", "Dokumentasi proses"] },
  { name: "Paket Bahagia", price: "Mulai 3,5 jt", detail: "Untuk 2 ekor kambing", accent: "orange", items: ["Kambing jantan & betina pilihan", "Pilihan menu lebih lengkap", "Kemasan premium keluarga", "Antar area Tangerang Selatan"] },
  { name: "Paket Berkah", price: "Konsultasikan", detail: "Untuk acara bersama", accent: "clay", items: ["Jumlah ekor fleksibel", "Menu custom sesuai kebutuhan", "Koordinasi distribusi", "Pendampingan dari awal"] },
];
  ...
  return (
    <div className="site-shell">
      ...
      <main id="beranda">
        ...
        <section className="packages-section section-pad" id="paket"><div className="packages-heading"><div><div className="section-kicker">04 — Pilih yang paling pas</div><h2>Paket aqiqah yang <em>siap membantu.</em></h2></div><p>Belum yakin memilih? Ceritakan jumlah tamu dan tanggal acara, kami bantu arahkan.</p></div><div className="packages-grid">{packages.map((pkg) => <article className={`package-card ${pkg.accent}`} key={pkg.name}><div className="package-top"><span className="package-tag">{pkg.accent === "orange" ? "Paling dipilih" : "Paket aqiqah"}</span><span className="package-arrow"><ArrowUpRight size={19} /></span></div><h3>{pkg.name}</h3><div className="package-price">{pkg.price}</div><p className="package-detail">{pkg.detail}</p><ul>{pkg.items.map((item) => <li key={item}><Check size={15} />{item}</li>)}</ul><a href={whatsappUrl} target="_blank" rel="noreferrer">Konsultasikan paket <ArrowUpRight size={15} /></a></article>)}</div></section>
