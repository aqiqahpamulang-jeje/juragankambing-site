...
export default function Home() {
  ...
  const safeAdditionalCost = Math.max(0, Number(additionalCost) || 0);
  ...
  return (
    <div className="site-shell">
      ...
      <main id="beranda">
        ...
        <section className="calculator-section section-pad" id="kalkulator"><div className="calculator-heading"><div className="section-kicker">05 — Hitung kebutuhanmu</div><h2>Kalkulator estimasi <em>biaya aqiqah.</em></h2><p>Atur jumlah kambing dan kebutuhan tambahan untuk mendapatkan gambaran biaya sebelum berkonsultasi dengan kami.</p></div><div className="calculator-card"><div className="calculator-form"><label>Jumlah ekor kambing<input type="number" min="1" value={animalCount} onChange={(event) => setAnimalCount(Math.max(1, Number(event.target.value) || 1))} /></label><label>Pilih paket<select value={selectedPackage} onChange={(event) => setSelectedPackage(event.target.value)}>{packages.map((pkg) => <option key={pkg.name} value={pkg.name}>{pkg.name.replace("Aqiqah 1 ekor ", "")} — {pkg.price}/ekor</option>)}</select></label><label>Biaya tambahan <span>(opsional)</span><input type="number" min="0" value={additionalCost} onChange={(event) => setAdditionalCost(Math.max(0, Number(event.target.value) || 0))} /></label><label>Diskon <span>(opsional)</span><input type="number" min="0" value={discount} onChange={(event) => setDiscount(Math.max(0, Number(event.target.value) || 0))} /></label></div><div className="calculator-summary"><div className="summary-row"><span>Total hewan</span><strong>{safeAnimalCount} ekor</strong></div><div className="summary-row"><span>Harga per ekor</span><strong>{formatCurrency(selectedPackageData.unitPrice)}</strong></div><div className="summary-row"><span>Subtotal</span><strong>{formatCurrency(subtotal)}</strong></div><div className="summary-row"><span>Biaya tambahan</span><strong>{formatCurrency(safeAdditionalCost)}</strong></div><div className="summary-row"><span>Diskon</span><strong>- {formatCurrency(safeDiscount)}</strong></div><div className="summary-total"><span>Total estimasi</span><strong>{formatCurrency(estimatedTotal)}</strong></div><a className="calculator-cta" href={confirmationUrl} target="_blank" rel="noreferrer">Konfirmasi &amp; hubungi kami <ArrowUpRight size={16} /></a></div></div></section>
